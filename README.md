...
- **Prisma 5.22.0** - ORM حديث وموثوق لإدارة قاعدة البيانات
...
│   │   ├── database.ts       # اتصال Prisma Client
│   │   ├── db_utils.ts       # دوال قاعدة البيانات (Prisma)
│   │   ├── ability.ts        # إعداد CASL
│   │   ├── roleManager.ts    # إدارة الأدوار
│   │   └── ...
│
├── prisma/                    # Prisma Configuration
│   ├── schema.prisma         # Prisma Schema (تعريف الجداول)
...
3. **إعداد قاعدة البيانات (Neon PostgreSQL)**
   ...
   المشروع يستخدم **Neon PostgreSQL** (قاعدة بيانات سحابية). تم إعداد ملف `.env` مسبقاً مع معلومات الاتصال.

   إذا كنت تريد استخدام قاعدة بيانات محلية:
   - تأكد من تثبيت PostgreSQL وإنشاء قاعدة بيانات جديدة
   - قم بتحديث `DATABASE_URL` في ملف `.env`

4. **إعداد متغيرات البيئة**

   ملف `.env` موجود بالفعل في جذر المشروع مع إعدادات قاعدة البيانات.

   لإضافة متغيرات إضافية (مثل NextAuth):

   ```env
   # Database Configuration (Neon PostgreSQL)
   DATABASE_URL="postgresql://neondb_owner:npg_sTKlgYh0z6RL@ep-proud-glade-ahq3hwos-pooler.c-3.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"

   # NextAuth Configuration (أضف هذه المتغيرات)
   ...
   لإنشاء `NEXTAUTH_SECRET` عشوائي:
   ```bash
   openssl rand -base64 32
   ```

5. **إعداد قاعدة البيانات مع Prisma**
...
pnpm db:push          # دفع Schema إلى قاعدة البيانات (للتطوير)
pnpm db:migrate       # إنشاء migration جديد
pnpm db:migrate:deploy # تطبيق migrations (للإنتاج)
pnpm db:studio        # فتح Prisma Studio (واجهة قاعدة البيانات)
...
1. **تثبيت PostgreSQL**
   - على Windows: قم بتحميل وتثبيت من [postgresql.org](https://www.postgresql.org/download/windows/)
   - على macOS: `brew install postgresql`
   - على Linux: `sudo apt-get install postgresql` (Ubuntu/Debian)
...
3. **إعداد متغيرات البيئة**
   - قم بإعداد `DATABASE_URL` في `.env.local`:
   ```env
